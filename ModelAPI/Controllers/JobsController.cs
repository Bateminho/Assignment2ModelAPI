...
public class JobsController : ControllerBase
{
	...
	{
		var allJobsWithModelNames = new List<JobWithModelNamesDto>();
		...
		foreach (var job in _context.Jobs)
		{
			...
			foreach (var model in job.Models) aJobWithModelNames.ModelNames.Add($"{model.FirstName} {model.LastName}");
	...
	public async Task<ActionResult<JobWithExpensesDto>> GetJobWithExpenses(long jobId)
	{
		...
		foreach (var expense in dbJob.Expenses) jobWithExpenses.Expenses.Add(expense.Adapt<ExpenseIdDto>());
	...
	public async Task<ActionResult<Job>> DeleteJob(long jobId)
	{
		...
		if (job == null) return NotFound("Did not find the job");
